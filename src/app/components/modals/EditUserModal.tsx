...
import { setOpenUserEditModal, setShowPassword } from '@/app/lib/features/commonFeaturesSlice';
...
const EditUserModal = () => {
    ...
    const { data: users, refetch } = useGetDataQuery('/users');
    const dispatch = useAppDispatch();
    const { showPassword, openUserEditModal, singleDataId } = useAppSelector((state) => state.commonFeatures)
    //GEt single order
        ...
        <>
            {
                openUserEditModal && <div className={`w-full min-h-screen flex items-center justify-center absolute py-10 left-0 top-0 z-50 bg-gray-900 bg-opacity-60`}>
                    <div className='w-11/12 md:w-2/5  flex items-center bg-gray-700 text-white p-5 my-5 md:my-0 rounded-md relative'>
                        <button
                            onClick={() => dispatch(setOpenUserEditModal())}
